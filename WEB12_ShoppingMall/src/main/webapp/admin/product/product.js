...
function go_search(){
	...
	var url ="shop.do?command=adminProductList&page=1";  //검색결과 1페이지 부터 봐야지!
...
function go_search_order(){
	if(document.frm.key.value=="") return;
	var url ="shop.do?command=adminOrderList&page=1";  //검색결과 1페이지 부터 봐야지!
	// 보던 페이지가 어떤 페이지 이드라도, 검색 결과의 1페이지로 가기 위해 파라미터 page 를 1로 전송
	document.frm.action = url;
	document.frm.submit();
}
...
function go_search_qna(){
	if(document.frm.key.value=="") return;
	var url ="shop.do?command=adminQnaList&page=1";  //검색결과 1페이지 부터 봐야지!
	// 보던 페이지가 어떤 페이지 이드라도, 검색 결과의 1페이지로 가기 위해 파라미터 page 를 1로 전송
	document.frm.action = url;
	document.frm.submit();
}
