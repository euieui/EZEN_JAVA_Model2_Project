...
<c:if test="${empty loginUser }">
	<jsp:forward page="board.do?command=loginFrom"></jsp:forward>
</c:if>
...
<div id="wrap" align="center">
	...
	<table>
		...
		<c:forEach var="board" items="${boardList }">
			<tr align="center">
				<td>${board.num }</td>
				<td align="left"><a href="board.do?command=boardView&num=${board.num}">${board.title }</a>&nbsp;
	...
	<div id="paging" align="center" style="font-size:110%; font-weight:bold;">
		...
		<c:forEach begin="${paging.beginPage }" end="${paging.endPage }" var="index">
			<c:choose>
				...
				<c:otherwise>
					<!-- 그 외에는 표시되는 숫자의 페이지를 파라미터로 해서 링크 걸어 표시 -->
